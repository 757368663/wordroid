...
public class OperationOfBooks {
	...
	public boolean ImportBook(Context context,String FileName,String NewName,int NumOfEachList,String order) throws Exception{
		...
		int ID=1;
		int listnum=1;
//		Log.e("now", "ImportBook: "+ifContinue );
//		Log.e("now", "ImportBook: "+br.readLine() );
		//从文件中获取单词  具体格式看下文
		while((line=br.readLine())!=null){
			if (ifContinue){    //每一单元的个数
				...
				Log.i("books111", "ImportBook: "+word.getID() + word.getSpelling()  +word.getPhonetic_alphabet());
				word.setMeanning(st.nextToken()+"."+st.nextToken());
//				Log.i("books111", "ImportBook: "+st.nextToken()+st.nextToken()+st.nextToken());
				list.add(word);
...
//			}
	...
	public void setNotify(String time,Context context){
		...
		Log.i("hour", "Operation"+hour);
		Log.i("minute", ""+minute);
		Calendar calendar=Calendar.getInstance();
		//如果传来的时间大于一天的时间或者等于一天的时间并且最大分钟数大于当前分钟。则天数加一
		...
		Log.i("format", f.format(calendar.getTime()));
		...
		am.cancel(sender);
		//设置 定时任务，向系统申请在18.00时发送该intent广播
		Intent intent2 = new Intent(context,makeNotify.class);
//		intent2.setComponent(new ComponentName("fun.eriri.wordroid","fun.eriri.wordroid.makeNotify"));
